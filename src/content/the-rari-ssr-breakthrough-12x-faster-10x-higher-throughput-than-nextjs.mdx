...
export const date = "2025-10-22"
export const tags = ["Open Source", "React", "Rust", "Architecture", "Performance"]

Three months ago, I shipped rari with solid numbers: 4x faster component rendering than Next.js, 3.74x higher throughput, and 5.8x faster builds. But the architecture still had gaps, most notably a missing app router, incomplete server-side rendering, and `use` directive semantics that weren't quite aligned with React's design.

So I went back and built those pieces properly. With app router support, true SSR, and correct RSC semantics in place, the performance profile changed significantly:
...
These aren't incremental improvements. Getting the architecture right, particularly around RSC semantics, changed the performance profile entirely.
...
I had the directives wrong, not broken exactly, but wrong enough to cause confusion.

`'use server'` doesn't mark server *components*. It marks server *functions*. Server components are the default and don't need a directive at all.
...
- **Client-side hydration overhead**, because the tree doesn't re-render to become interactive
- **JavaScript parsing delay**, since interactive components are already wired up
- **Cascading waterfall requests**, because data is fetched server-side during render
...
These gains come from correct architecture and proper semantics reinforcing each other.

**Smaller Bundles (68% reduction):** Only interactive components ship to the client while server components stay on the server. The app router eliminates dead code, so the framework knows exactly what the client actually needs.

**Lower Latency (3.8x faster):** SSR means no client-side hydration wait. The Rust runtime renders faster than JavaScript, and RSC rendering starts immediately with no layers between components and the client.
...
**Predictable Performance:** No garbage collection pauses, no event loop blocking, and linear scaling with more instances.

## What This Means in Practice

**For users:** HTML arrives immediately from the server. Interactive components hydrate instantly, and the client only runs code for interactive features. The server handles the heavy lifting under load.

**For developers:** `'use server'` and `'use client'` directives work as expected. HMR works for both server and client components, with full TypeScript support across the client/server boundary. Build, bundle, and RSC setup are automatic.

**For infrastructure:** Smaller client JavaScript means fewer resources needed. The Rust runtime handles 10.5x more concurrent requests with no garbage collection pauses under load.
