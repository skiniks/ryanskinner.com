...
<template>
  ...
  <div class="mx-auto px-4 sm:px-6 lg:px-8 min-h-screen">
    <div class="mx-auto sm:max-w-xl md:max-w-2xl lg:max-w-4xl">
      <div class="space-y-8">
        <div v-if="entries.length > 0" :key="entries[0].path" class="flex flex-col items-start justify-between bg-gray-900 light:bg-gray-800/80 rounded-lg px-6 py-8 sm:px-8 sm:py-10 md:px-12 md:py-14 my-4 shadow-md transition-all ease-in-out duration-300 hover:shadow-lg animate-fadeInMoveUp">
          <div class="mb-4">
          ...
          <a :href="entries[0].path" class="text-white bg-blue-600 hover:bg-blue-800 px-4 py-4 rounded-xl" :aria-label="'Read more about ' + entries[0].title" :title="'Read more about ' + entries[0].title">
            View Post
          </a>
        </div>
